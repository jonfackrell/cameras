...
@section('toolbox')
    ...
    <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right top_search" style="margin-bottom: 0px;">
        {!! BootForm::open()->action(route('3d.admin'))->get() !!}
...
@section('content')
                        ...
                        <tbody>
                            @foreach($printJob as $row)
                                <tr data-id="{{ $row->id }}">
                                    ...
                                    <td>
                                        {!! BootForm::open()->action(route('3d.admin.edit', $row->id ))->get() !!}
                                        {!! BootForm::select('Status', 'status')->options($statuses->pluck('name', 'id'))->select($row->status)->hideLabel()->addClass('status-update input-sm') !!}
                                        {!! BootForm::close() !!}
                                    </td>
                                    <td>
                                        <a href="{{ route('3d.uploadfile.edit', $row->id) }}" class="btn btn-info btn-sm" title="Edit"><i class="fa fa-pencil"></i></a>
                                        <a href="{{ route('3d.download', $row->filename) }}" class="btn btn-success btn-sm" title="Download {{ $row->original_filename }}"></i><i class="fa fa-download"></i></a>
                                        <a href="{{ route('3d.admin.create-email', $row->id) }}" class="btn btn-warning btn-sm" title="Email"></i><i class="fa fa-envelope"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
    ...
    @else
        ...
        <div class="alert alert-info">
            <p>
                You do not currently have any Statuses set up for your department to display. <a href="{{ route('3d.status.index') }}" class="btn btn-warning btn-xs">Manage Statuses</a>
            </p>
        </div>
